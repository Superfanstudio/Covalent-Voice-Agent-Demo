...
export async function route({ method, pathname, query, body, headers = {} }) {
  ...
  try {
    ...
    // ---- saved simulator scenarios (Finance org simulator, extensible by `tool`) ----
    if (head === "scenarios") {
      const id = parts[1];
      try {
        // GET /api/scenarios?tool=finance_org → lightweight list for the selector
        if (!id && method === "GET") {
          return ok({ scenarios: await scenarios.listScenarios(query?.tool || "finance_org") });
        }
        // GET /api/scenarios/:id → one scenario with its full state
        if (id && method === "GET") {
          const s = await scenarios.getScenario(id);
          if (!s) return { status: 404, json: { error: "No such scenario" } };
          return ok({ scenario: s });
        }
        // POST /api/scenarios {tool, name, created_by, based_on, state} → save a version
        if (!id && method === "POST") {
          const s = await scenarios.createScenario({
            tool: body?.tool || "finance_org",
            name: body?.name,
            created_by: body?.created_by,
            based_on: body?.based_on,
            state: body?.state,
          });
          capture("scenario_saved", { tool: s.tool, based_on: !!s.based_on }, body?.created_by);
          return ok({ scenario: s });
        }
      } catch (err) {
        if (err?.code === "bad_input") return bad(err.message);
        if (err instanceof NotConfigured) return { status: 503, json: { error: err.message, code: "supabase_not_configured" } };
        throw err;
      }
    }
    ...
    if (head === "versions") {
      ...
      // POST /api/versions/publish — publish an externally-authored HTML doc as
      // the next version (access-code gated; preserves the seed as v1).
      if (sub === "publish" && method === "POST") {
        try {
          const res = await versions.publishHtml({
            dept: body?.dept,
            accessCode: body?.access_code,
            html: body?.html,
          ...
          capture("version_published", { dept: res.dept, version: res.version }, body?.created_by);
          return ok(res);
        } catch (err) {
          if (err?.code === "bad_access_code") return { status: 403, json: { error: err.message, code: err.code } };
          if (err?.code === "bad_dept" || err?.code === "bad_html") return { status: 400, json: { error: err.message, code: err.code } };
          if (err?.code === "not_configured") return { status: 503, json: { error: err.message, code: err.code } };
          throw err;
        }
      }
