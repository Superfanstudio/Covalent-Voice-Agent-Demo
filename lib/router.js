  ...
  try {
    ...
    if (head === "health" && method === "GET") {
      ...
      return ok({
        ...
        ready: env.assemblyai_key && env.supabase && env.anthropic_key && env.version_access_code && schema === "ok",
        hint: schema !== "ok"
          ? `Point SUPABASE_URL at https://${expected} and use that project's service-role key, then redeploy.`
          : (env.anthropic_key && env.version_access_code ? "All set — seed the KB from /admin if you haven't." : "Add ANTHROPIC_API_KEY and VERSION_ACCESS_CODE to enable versions, Agent Mode, and memory."),
      });
    }
