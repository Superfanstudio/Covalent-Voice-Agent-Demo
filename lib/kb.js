...
export async function ingest({ title, text, source = "paste", dept = null }) {
  ...
  const { data: doc, error: docErr } = await sb
    .from("kb_documents")
    .insert({ title: cleanTitle, source, dept: dept || null, char_count: String(text).length })
    .select("id, title, source, dept, char_count, created_at")
    .single();
...
export async function search(query, limit = 5, dept = null) {
  const q = String(query || "").trim();
  if (!q) return [];
  const sb = requireSupabase();
  const { data, error } = await sb.rpc("search_kb", { q, k: limit, dept_filter: dept || null });
...
export async function listDocs({ dept = null } = {}) {
  ...
  let q = sb
    .from("kb_documents")
    .select("id, title, source, dept, char_count, created_at, kb_chunks(count)")
