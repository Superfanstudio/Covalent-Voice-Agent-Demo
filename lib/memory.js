...
async function runFable(userPrompt, feature = "memory") {
  ...
  captureGeneration({
    model: MEMORY_MODEL,
    input: [{ role: "system", content: MEMORY_SYSTEM }, { role: "user", content: userPrompt }],
    output: text.trim(),
    usage: message.usage,
...
Return the COMPLETE updated memory document (not a diff).`;
  ...
  const { text, usage } = await runFable(prompt, "memory_update");
  if (!text) return null;
