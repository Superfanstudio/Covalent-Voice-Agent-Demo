...
export async function createConversation({ agent, voice, client_id }) {
  const sb = requireSupabase();
  const { data, error } = await sb
    .from("conversations")
    .insert({ agent, voice, client_id, status: "live" })
    .select("id")
    .single();
...
export async function listConversations({ limit = 100 } = {}) {
  ...
  const { data, error } = await sb
    .from("conversations")
    .select("id, agent, voice, status, started_at, ended_at, turns(count)")
...
export async function getConversation(id) {
  ...
  const { data: conv, error: convErr } = await sb
    .from("conversations")
    .select("id, agent, voice, status, started_at, ended_at, client_id")
    .eq("id", id)
    .single();
